...
print(find_unusual_entries(badge_times))
...
find the largest group of people that were in the room together during two or 
more separate time periods, and the times when they were all present.
badge_records = [
["Paul", "1214", "enter"],
["Paul", "830", "enter"],
["Curtis", "1100", "enter"],
...
The group "Jennifer, John, Paul" exists at both of these times, and is the largest 
group that exists multiple times.
...
def largest_group(logs):
  ...
  for timeint, event, username in records:
    if event == 0:
      # print(username, user_id[username])
      state |= 1 << user_id[username] # 向左移动user_id[username]位
    else:
