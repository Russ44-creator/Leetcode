...
'''
'''
part 2 是问如果client可以直接specify language需要怎么去修改第一轮的答案。example:
parseacceptlanguage(
"en, fr-CA, fr-FR", # the client's Accept-Language header, a string
["fr-FR", "en-US"] # the server's supported languages, a set of strings
) -> ["en-US", "fr-CA", "fr-FR"]
注意这里要按照client preference。
part 3 是问如果client可以有 "*"来表示client takes any language。part 3挺简单的。没时间做part 4但还是过了。
'''
